...
| 模块       | 文件                                  | 职责                                                                                                                                                                             |
| ---------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 入口       | `main.rs`                             | 加载配置 → 初始化日志 → 根据 `[im.*]` 子表创建 `Arc<dyn IMChannel>` → 启动 `LinkService` + cron 调度器                                                                           |
...
| ACP 桥接   | `link/acp.rs`                         | agent 子进程池、`!Send` runtime 隔离（`current_thread` + `LocalSet`）、FNV-1a 稳定 hash 路由、流式 chunk 转发、权限自动批准、worker 崩溃自动重启、keepalive 心跳（每 6 小时）   |
| 核心服务   | `link.rs`                             | 消息分发（每消息独立 task）、session 生命周期管理（全量/增量模式）、content block 构建、消息流式更新节流（300ms）、定时清理、优雅关机                                            |
| 定时调度   | `link/cron.rs`                        | 根据 `[[cron]]` 配置定期触发 prompt → agent，流式响应以消息卡片发往目标会话；支持热重载（`watch::Receiver<Vec<CronJob>>`）                                                        |
...
| 配置管理   | `config.rs`                           | TOML 解析、`[im.*]` 子表自动识别平台、优先级查找（环境变量 > 当前目录 > `~/.acp-link/`）、自动生成默认配置模板、目录自动创建                                                     |
...
graph TD
    ...
    cron["link/cron.rs<br/>CronScheduler<br/>定时任务 + 热重载"]
...
- **Cron 调度器**：`link/cron.rs` 通过 `watch::Receiver<Vec<CronJob>>` 接收配置，按 cron 表达式定时触发 prompt → agent，流式响应推送到目标 IM 会话；配置变更时自动热重载调度器。
...
[im.feishu]                     # 配置哪个 IM 平台就用哪个（目前支持 feishu）
app_id     = "cli_xxx"
app_secret = "your_secret"

[backend]                       # 任意 ACP 兼容 agent（kiro-cli、claude 等）
cmd       = "kiro-cli"
args      = ["acp", "--agent", "lark"]
...
# 定时任务（可配置多个）
[[cron]]
schedule    = "0 9 * * 1-5"    # 标准 5 位 cron：每个工作日 09:00
prompt      = "请生成今日工作计划摘要"
target_id   = "oc_xxxxxxxx"    # 目标会话 ID（飞书 chat_id）
target_type = "p2p"            # "p2p"（单聊）或 "group"（群聊），默认 p2p
```

`[im.*]` 子表的存在性决定使用哪个 IM 平台，`main.rs` 自动识别并创建对应的 `Arc<dyn IMChannel>`，无需额外的 `im_platform` 字段。
...
| tokio-cron-scheduler         | 0.15 (signal)          | Cron 表达式调度（quartz 格式）     |
| dirs                         | 6.0                    | 获取用户主目录                     |
