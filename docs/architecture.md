...
| 飞书客户端 | `im/feishu/client.rs`（`pub(crate)`） | WS 长连接、protobuf 帧解析、消息分片重组、消息去重、token 缓存、REST API（回复/更新消息、上传/发送文件、下载资源、拉取 thread、获取云文档）                                      |
...
| `feishu_get_document` | 获取飞书云文档纯文本内容（支持完整 URL 或裸 document_id） | `document`（必填）                                             |
